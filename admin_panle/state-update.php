...
if (isset($_GET['s_id'])) {
  $s_id = $_GET['s_id'];
  $s_query = "SELECT * FROM state WHERE id='$s_id'";
        ...
        <div class="container">
          ...
          <form action="" method="post">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" name="state" id="exampleFormControlInput1" value="<?php echo $row['s_name']; ?>" placeholder="Country">
            </div>
            <div class="mb-3">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['submit'])) {
    if ($state == !"") {
      ...
      //  }
      //  else{
       $query = "UPDATE `state` SET `co_id` = '$country',`s_name` = '$state' WHERE `state`.`id` = $s_id"; 
        $s_run = mysqli_query($conn, $query);
       if($s_run){

         echo "<script>alert('$s_name State Name Is Change To $state')</script>";
         ?>
         <script>
       ...
       {
         echo "<script>alert('$s_name State Name Is Change To $state Not')</script>";
       }
