        ...
        <div class="container">
          ...
          <form action="" method="post">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" name="state" id="exampleFormControlInput1"  placeholder="City">
            </div>
            <div class="mb-3">
     ...
     $s_state = trim($_POST['state']);
      $co_id = $_POST['co_id'];
       if(isset($_POST['submit']))
       {
          if($s_state == !"")
          {
             $select = "SELECT * FROM city WHERE name='$s_state'";
             $s_run = mysqli_query($conn,$select);
             $one = mysqli_num_rows($s_run);
             if($one > 0)
             {

                 echo "<script>alert('City Name Already Add')</script>";
         ...
         {
            echo $one;
        $query = "INSERT INTO `city`(`s_id`,`name`) VALUES ('$co_id','$s_state')";
        $run = mysqli_query($conn,$query);
        echo "<script>alert('City Name Insert')</script>";
        ?>
         <script>
